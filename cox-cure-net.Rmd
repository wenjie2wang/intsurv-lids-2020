...
`cox_cure_net.fit()`{.R} provided by \proglang{R} package \pkg{intsurv}
[@intsurv-package] for fitting regularized Cox cure rate model with elastic-net
penalty [@zouHastie2005jrssb].


The cure rate models first proposed by @berkson1952jasa are commonly adopted
...
Proposed by @farewell1982biometrics, a logistic model
...
Given that $Z_j = 1$, @kuk1992biometrika proposed modeling the conditional
survival times through a Cox proportional hazard model <!-- [@cox1972jrssb] -->
...
A model estimation procedure based on the well-known EM
algorithm <!-- [@dempster1977jrssb] --> was proposed by @sy2000biometrics.
Recently, a few works have been proposed to perform variable selection for cure
models.  For example, @scolas2016sim proposed variable selection with adaptive
lasso penalty [@zou2006jasa] for interval-censored data in a parametric cure
model, where conditional survival times follow the extended generalized gamma
distribution.  @masud2018smimr proposed variable selection methods for mixture
cure model and promotion cure model through regularization by the adaptive lasso
penalty.  @fan2017smmr and @shi2019smmr promoted structural similarity and sign
consistency of $\hat{\bg}$ and $\hat{\bb}$, respectively, with minimax concave
penalty [@zhang2010aos] for variable selection.
...
\end{align*}
where $\omega_k$ and $\nu_k$ represent non-negative weights [@zou2006jasa],
$0\le\alpha_1\le1$, $0\le\alpha_2\le1$, $\lambda_1\ge0$, and $\lambda_2\ge0$
are tuning parameters.
The coordinate descent (CD) algorithm [@friedman2007aoas] or local quadratic
approximations [@fanLi2001jasa] may be utilized in the M-steps of the EM
...
algorithm proposed by @yang2013sii in the M-steps due to its descent property.
...
setting one-step CD update as follows.  In this example, we are able to decrease
the computation time by 80% and obtain the same variable selection results.
